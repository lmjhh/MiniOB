...
private:
  ...
  PageNum page_num_ = INVALID_PAGE_NUM;
...
public:
  ...
  ~RecordPageHandler();
  RC init(int file_desc, PageNum page_num);
  RC init_empty_page(int file_desc, PageNum page_num, int record_size);
  RC cleanup();
...
public:
   ...
   */
  RC open_scan(int file_id, ConditionFilter *condition_filter);
...
private:
  int file_desc_ = -1;
  DiskFileIterator file_iterator_;
