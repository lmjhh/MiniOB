...
const static Json::StaticString FIELD_NULLABLE("isNullable");
...
void FieldMeta::desc(std::ostream &os) const {
  os << "field name=" << name_
     ...
     << ", is_nullable=" << (is_nullable_ ? "yes" : "no");
}
...
void FieldMeta::to_json(Json::Value &json_value) const {
  ...
  json_value[FIELD_NULLABLE] = is_nullable_;
}
...
RC FieldMeta::from_json(const Json::Value &json_value, FieldMeta &field) {
  ...
  bool is_nullable = nullable_value.asBool();
