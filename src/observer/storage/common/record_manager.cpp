...
RC RecordPageHandler::init(int file_desc, PageNum page_num)
{
  if (file_desc_ != -1) {
  ...
  RC rc = RC::SUCCESS;
  if ((rc = MemBufferPoolManager::instance().get_this_page(file_desc, page_num, &frame_)) != RC::SUCCESS) {
...
RC RecordPageHandler::init_empty_page(int file_desc, PageNum page_num, int record_size)
{
  RC ret = init(file_desc, page_num);
  if (ret != RC::SUCCESS) {
    LOG_ERROR("Failed to init empty page page_num:record_size %d:%d.", page_num, record_size);
    return ret;
  }

  int page_size = PAGE_DATA_SIZE;
...
{
  if (file_desc_ != -1) {
    MemBufferPoolManager::instance().unpin_page(frame_);
    file_desc_ = -1;
  }
  return RC::SUCCESS;
}
...
RC RecordPageHandler::delete_record(const RID *rid)
{
  ...
  if (bitmap.get_bit(rid->slot_num)) {
    ...
    if (page_header_->record_num == 0) {
      ...
      cleanup();
      MemBufferPoolManager::instance().dispose_page(file_desc_ , page_num);
    }
    return RC::SUCCESS;
  } else {
    LOG_ERROR("Invalid slot_num %d, slot is empty, page_num %d.",
...
RC RecordFileHandler::init(int file_desc)
{
  if (file_desc_ != -1) {
...
RC RecordFileHandler::insert_record(const char *data, int record_size, RID *rid)
{
  ...
  RC ret = RC::SUCCESS;
  // 找到没有填满的页面
  DiskFileIterator file_iterator;
  file_iterator.init(file_desc_);
  RecordPageHandler record_page_handler;
  bool page_found = false;
  PageNum current_page_num = 0;
  while (file_iterator.has_next()) {
    current_page_num = file_iterator.next();
    ret = record_page_handler.init(file_desc_, current_page_num);
    if (ret != RC::SUCCESS) {
  ...
  if (!page_found) {
    Frame *frame = nullptr;
    if ((ret = MemBufferPoolManager::instance().allocate_page(file_desc_, &frame)) != RC::SUCCESS) {
      LOG_ERROR("Failed to allocate page while inserting record. ret:%d", ret);
      return ret;
    }
    current_page_num = frame->page_num();
    ret = record_page_handler.init_empty_page(file_desc_, current_page_num, record_size);
    if (ret != RC::SUCCESS) {
      LOG_ERROR("Failed to init empty page. ret:%d", ret);
      if (RC::SUCCESS != MemBufferPoolManager::instance().unpin_page(frame)) {
        LOG_ERROR("Failed to unpin page. ");
      }
      return ret;
    }
    MemBufferPoolManager::instance().unpin_page(frame);
  }

  // 找到空闲位置
  ret = record_page_handler.insert_record(data, rid);
...
RC RecordFileHandler::update_record(const Record *rec)
{
  ...
  if ((ret = page_handler.init(file_desc_, rec->rid().page_num)) != RC::SUCCESS) {
...
RC RecordFileHandler::delete_record(const RID *rid)
{
  RC ret = RC::SUCCESS;
  RecordPageHandler page_handler;
  if ((ret != page_handler.init(file_desc_, rid->page_num)) != RC::SUCCESS) {
...
RC RecordFileScanner::open_scan(int file_desc, ConditionFilter *condition_filter)
{
  close_scan();

  file_desc_ = file_desc;

  RC rc = file_iterator_.init(file_desc);
...
RC RecordFileScanner::fetch_next_record()
{
  ...
  while (file_iterator_.has_next()) {
    PageNum page_num = file_iterator_.next();
    record_page_handler_.cleanup();
    rc = record_page_handler_.init(file_desc_, page_num);
    if (rc != RC::SUCCESS) {
